...
test("service create latest test",()=>{
    const weight = WeightService.createWeight("123",67,file=testDataFile);
    expect(weight.userId).toBe("123");
    expect(weight.weight).toBe(67);
    const weight_1 = WeightService.getLatestWeight("123",file);
    expect(weight_1.id).toBe(weight.id)
    const weight_2 = WeightService.getLatestWeight("none",file);
